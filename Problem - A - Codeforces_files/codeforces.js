...
function getLocalizedName(localized) {
    const blankEnglishName = !localized.englishName || localized.englishName.length === 0;
    const blankRussianName = !localized.russianName || localized.russianName.length === 0;
    ...
    if (englishLocale) {
        ...
        if (!blankRussianName) {
            return localized.russianName;
        }
    }

    const russianLocale = locale === "ru";
    if (russianLocale) {
        if (!blankRussianName) {
            return localized.russianName;
        }
...
function getLocalizedDescription(localized) {
    const blankEnglishDescription = !localized.englishDescription || localized.englishDescription.length === 0;
    const blankRussianDescription = !localized.russianDescription || localized.russianDescription.length === 0;

    const locale = window.locale;

    const englishLocale = locale === "en";
    if (englishLocale) {
        if (!blankEnglishDescription) {
            return localized.englishDescription;
        }

        if (!blankRussianDescription) {
            return localized.russianDescription;
        }
    }
...
Codeforces = new function () {
    ...
    this.countdown = function () {
        ...
        var callback = function () {
            countdownElements.each(function () {
                ...
                var enableAnotherOnCountdownFinish = countdownElement.attr("enableAnotherOnCountdownFinish");
                var enableAnotherClass = countdownElement.attr("enableAnotherClass");
                var removeAnotherOnCountdownFinish = countdownElement.attr("removeAnotherOnCountdownFinish");
                var removeAnotherClass = countdownElement.attr("removeAnotherClass");
                ...
                if (s > 0) {
                    ...
                    if (val <= 0) {
                        ...
                        if (enableAnotherOnCountdownFinish == "true" || enableAnotherOnCountdownFinish == "yes"
                            || enableAnotherOnCountdownFinish == "y" || enableAnotherOnCountdownFinish == "1") {
                            $("." + enableAnotherClass).prop("disabled", false);
                        }

                        if (removeAnotherOnCountdownFinish == "true" || removeAnotherOnCountdownFinish == "yes"
                            || removeAnotherOnCountdownFinish == "y" || removeAnotherOnCountdownFinish == "1") {
                            $("." + removeAnotherClass).remove();
                        }
    ...
    this.alert = function (messageHtml, okHandler, okText, props) {
        Codeforces.confirm(messageHtml, okHandler, null, okText, null, false, props);
    };

    this.confirm = function (messageHtml, okHandler, cancelHandler, okText, cancelText, noClose, props) {
        if (typeof (noClose) === 'undefined') {
            noClose = false;
        }

        props = props || {};
        const clazz = props.clazz ?? "text";

        const dialogHtml = ("<div class='confirm-proto'>"
            + "<div class='confirm-dialog-text " + this.escapeHtml(clazz) + "'>&nbsp;<\/div>"
            + "<div>"
            + "<hr\/>"
            + "<div style='text-align:center;'>"
            + "<input class='ok' name='codeforces-dialog-ok-button' type='button' value='%OK%'\/>"
            + "<input class='cancel' name='codeforces-dialog-cancel-button' type='button' value='%Cancel%'\/>"
            + "<\/div>"
            + "<\/div>"
            + "<\/div>")
            .replace("%OK%", okText ? okText : "OK")
            .replace("%Cancel%", cancelText ? cancelText : "Cancel");
        const p = $(dialogHtml).css('display', 'none');
        $(".confirm-dialog-text", p).html(messageHtml);

        let defaultButtonName = null;

        const cancelButton = $(".cancel", p);
        if (cancelHandler) {
...
//            window.localStorage[key] = defaultValue;
    ...
    this.setupContestTimes = function (contestDataPageUrl, elem) {
        ...
        elem.find(".contest-time").each(function () {
            const $this = $(this);
            const contestId = $(this).attr("contestId");
            $.post(contestDataPageUrl, {
                action: "getContestTime",
                contestId: contestId
            }, function (result) {
                if (result["success"] === "true") {
                    const href = result["contestTimeHref"];
                    const html = result["contestTimeHtml"];
                    $this.attr("href", href);
                    $this.html(html);
    ...
    this.checkInternetRestriction = function (url, callback) {
        const key = "cirlrt:" + url; // Check Internet Restriction Last Request Time
        const lastRequestTime = parseInt(localStorage.getItem(key));
        ...
        if (!lastRequestTime
            ...
            const timedUrl = appendParameterToUrl(url, "t", currentTime);
            fetch(timedUrl)
                .then(response => {
                    if (response.ok) {
                ...
                .then(data => {
                    if (data.indexOf("0b362253c64368d6eebc50ea21317") >= 0) {
                        callback(url); // Call the callback function with the URL
                    }
                })
                .catch(error => {
                    console.error("Error during GET request [url=" + url + "]:", error);
                });
        } else {
            console.log("Last request was made less than delay [url=" + url + "]: "
                + (currentTime - lastRequestTime) + " ms");
        }
    }
};
...
$(document).ready(function () {
    // Append the popup container dynamically
    $("body").append(`
    ...
    // Handle click on any image with class 'resource'
